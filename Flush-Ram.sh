...
echo "  _ _                  /            ";
echo "  \\/isit:  github.com/mayur-chavhan";
echo "                                    ";

# os needs to be debian and  ubuntu
if [ "$(lsb_release -is)" != "Debian" ] && [ "$(lsb_release -is)" != "Ubuntu" ]; then
    echo "This script is only for Debian and Ubuntu"
    exit 1
fi
...
# check if bc installed or not, If not then install it
